...
가격이 직전 구조 저점 또는 직전 구조 고점 근처까지 접근했지만, 해당 구조선을 실제로 돌파하지 못하고 긴 꼬리 핀바형 캔들로 반전 마감할 때 진입한다.

- 직전 저점 돌파 실패 + 긴 아래꼬리 + 양봉 마감 = BUY
- 직전 고점 돌파 실패 + 긴 위꼬리 + 음봉 마감 = SELL
...
4. 셋업캔들의 저가가 직전 저점을 하향 돌파하면 안 된다.
5. 셋업캔들의 저가는 직전 저점으로부터 `LevelTouchTolerancePoints` 이내에 있어야 한다.
6. 셋업캔들은 양봉으로 마감해야 한다.
7. 셋업캔들은 긴 아래꼬리 핀바형이어야 한다.
8. 현재 심볼과 매직넘버 기준 기존 포지션이 없어야 한다.
...
4. 셋업캔들의 고가가 직전 고점을 상향 돌파하면 안 된다.
5. 셋업캔들의 고가는 직전 고점으로부터 `LevelTouchTolerancePoints` 이내에 있어야 한다.
6. 셋업캔들은 음봉으로 마감해야 한다.
...
| 직전 저점 돌파 실패 BUY | 셋업캔들 저가가 직전 스윙 저점 아래로 내려가면 BUY 금지 |
| 직전 고점 돌파 실패 SELL | 셋업캔들 고가가 직전 스윙 고점 위로 올라가면 SELL 금지 |
