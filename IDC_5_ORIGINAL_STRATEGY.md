...
명확한 구조가 진행되다가 직전 스윙저점 또는 직전 스윙고점을 종가로 돌파하지 못하고, 긴 꼬리로 유동성을 확보한 뒤 몸통과 종가가 다시 구조 안쪽으로 복귀할 때 진입한다.

- LH/LL 하락 구조 진행 + 직전 스윙저점 하향 스윕 후 몸통/종가 복귀 + 긴 아래꼬리 + 양봉 마감 = BUY
- HL/HH 상승 구조 진행 + 직전 스윙고점 상향 스윕 후 몸통/종가 복귀 + 긴 위꼬리 + 음봉 마감 = SELL
...
5. 셋업캔들의 저가는 직전 스윙저점을 실제로 터치하거나 하향 스윕해야 한다.
6. 하향 스윕 폭은 `MaxLevelSweepPoints`를 초과하면 안 된다.
7. 셋업캔들의 몸통은 직전 스윙저점 위쪽으로 복귀해야 한다.
8. 셋업캔들의 종가는 직전 스윙저점 위로 복귀해야 한다.
...
5. 셋업캔들의 고가는 직전 스윙고점을 실제로 터치하거나 상향 스윕해야 한다.
6. 상향 스윕 폭은 `MaxLevelSweepPoints`를 초과하면 안 된다.
7. 셋업캔들의 몸통은 직전 스윙고점 아래쪽으로 복귀해야 한다.
8. 셋업캔들의 종가는 직전 스윙고점 아래로 복귀해야 한다.
...
| 직전 저점 돌파 실패 BUY | 셋업캔들 저가가 직전 스윙저점 터치/하향 스윕 후 몸통과 종가가 저점 위로 복귀 |
| 직전 고점 돌파 실패 SELL | 셋업캔들 고가가 직전 스윙고점 터치/상향 스윕 후 몸통과 종가가 고점 아래로 복귀 |
