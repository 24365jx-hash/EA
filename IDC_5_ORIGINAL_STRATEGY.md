...
- EA 이름: `IDC_5`
- 현재 버전: `1.05`
- 전략명: GOLD M1 최근 30캔들 고저점 실패돌파 핀바 전략
- 대상 차트: GOLD / M1
...
셋업캔들 직전 `SetupLookbackBars`개 캔들 안의 최저점 또는 최고점 심지 라인을 기준으로, 셋업캔들이 그 라인을 바늘처럼 실제로 찌르는 false breakout을 만들고 종가로는 돌파마감하지 못한 채 기준선 안쪽에서 마감할 때 진입한다.

- 직전 `SetupLookbackBars`개 캔들 중 최저점 아래로 실제 하향 스윕 후 그 라인 위에서 마감 + 긴 아래꼬리 + BUY 방향 컬러 확인 = BUY
- 직전 `SetupLookbackBars`개 캔들 중 최고점 위로 실제 상향 스윕 후 그 라인 아래에서 마감 + 긴 위꼬리 + SELL 방향 컬러 확인 = SELL
...
- 기준선은 셋업캔들 직전 `SetupLookbackBars`개 캔들 중 최저 저가 라인이다.
- 저가는 심지를 포함한다.
- 기본값은 `SetupLookbackBars = 30`이다.
- 셋업캔들 자체는 이 기준선 탐색에서 제외한다.

### 3.2 SELL 기준 라인

- 기준선은 셋업캔들 직전 `SetupLookbackBars`개 캔들 중 최고 고가 라인이다.
- 고가는 심지를 포함한다.
- 기본값은 `SetupLookbackBars = 30`이다.
- 셋업캔들 자체는 이 기준선 탐색에서 제외한다.
...
3. 셋업캔들 직전 `SetupLookbackBars`개 캔들 중 최저점 라인이 존재해야 한다.
4. 셋업캔들의 저가는 해당 최저점 라인을 `MinFalseBreakoutPoints` 이상 실제로 하향 이탈해야 한다.
5. 이탈 폭 상한 제한은 없다.
6. 셋업캔들의 종가는 해당 최저점 라인 위에서 마감해야 한다.
7. 셋업캔들은 긴 아래꼬리 핀바형이어야 한다.
8. 셋업캔들이 양봉이면 다음 새 봉 오픈 시점에 즉시 BUY 진입한다.
9. 셋업캔들이 음봉이면 셋업캔들을 포함해 3개 캔들 안에서 양봉이 마감될 때 다음 새 봉 오픈 시점에 BUY 진입한다.
10. 셋업캔들이 불리쉬 형태 도지, 즉 아래꼬리가 위꼬리보다 긴 도지이면 셋업캔들을 포함해 3개 캔들 안에서 양봉 확인을 기다린다.
11. 그 외 도지는 제외한다.
12. 현재 심볼과 매직넘버 기준 기존 포지션이 없어야 한다.
...
1. 차트 주기는 M1이어야 한다.
2. 새 봉이 시작된 시점에 직전 마감봉을 셋업캔들로 평가한다.
3. 셋업캔들 직전 `SetupLookbackBars`개 캔들 중 최고점 라인이 존재해야 한다.
4. 셋업캔들의 고가는 해당 최고점 라인을 `MinFalseBreakoutPoints` 이상 실제로 상향 이탈해야 한다.
5. 이탈 폭 상한 제한은 없다.
6. 셋업캔들의 종가는 해당 최고점 라인 아래에서 마감해야 한다.
7. 셋업캔들은 긴 위꼬리 핀바형이어야 한다.
8. 셋업캔들이 음봉이면 다음 새 봉 오픈 시점에 즉시 SELL 진입한다.
9. 셋업캔들이 양봉이면 셋업캔들을 포함해 3개 캔들 안에서 음봉이 마감될 때 다음 새 봉 오픈 시점에 SELL 진입한다.
10. 셋업캔들이 베어리쉬 형태 도지, 즉 위꼬리가 아래꼬리보다 긴 도지이면 셋업캔들을 포함해 3개 캔들 안에서 음봉 확인을 기다린다.
11. 그 외 도지는 제외한다.
12. 현재 심볼과 매직넘버 기준 기존 포지션이 없어야 한다.
...
| `SetupLookbackBars` | 셋업캔들 직전 기준 고점/저점 탐색 캔들 수 |
...
| BUY 기준 라인 | 셋업캔들 직전 `SetupLookbackBars`개 캔들 중 최저 저가 |
| SELL 기준 라인 | 셋업캔들 직전 `SetupLookbackBars`개 캔들 중 최고 고가 |
| 직전 저점 돌파 실패 BUY | 셋업캔들 저가가 기준 최저점 라인을 실제 하향 이탈 후 종가가 라인 위에서 마감 |
| 직전 고점 돌파 실패 SELL | 셋업캔들 고가가 기준 최고점 라인을 실제 상향 이탈 후 종가가 라인 아래에서 마감 |
