...
- 기준선은 직전 확정 저점이다.
- EA 구현에서는 `SwingDepthBars` 기준으로 좌우 봉보다 낮은 확정 스윙 저점을 사용한다.
- 최근 `StructureSearchBars` 범위 안에서 가장 가까운 확정 스윙 저점을 BUY 기준 구조선으로 본다.

### 3.2 SELL 기준 구조

- 기준선은 직전 확정 고점이다.
- EA 구현에서는 `SwingDepthBars` 기준으로 좌우 봉보다 높은 확정 스윙 고점을 사용한다.
- 최근 `StructureSearchBars` 범위 안에서 가장 가까운 확정 스윙 고점을 SELL 기준 구조선으로 본다.
...
| 직전 저점 돌파 실패 BUY | 셋업캔들 저가가 직전 스윙 저점 터치/하향 스윕 후 종가가 저점 위로 복귀 |
| 직전 고점 돌파 실패 SELL | 셋업캔들 고가가 직전 스윙 고점 터치/상향 스윕 후 종가가 고점 아래로 복귀 |
