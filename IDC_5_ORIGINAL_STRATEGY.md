...
확정 스윙저점 또는 확정 스윙고점 라인을 기준으로, 셋업캔들이 그 라인을 바늘처럼 실제로 찌르는 false breakout을 만들고 종가로는 돌파마감하지 못한 채 기준선 안쪽에서 마감할 때 진입한다.

- 직전 확정 스윙저점 아래로 실제 하향 스윕 후 그 라인 위에서 마감 + 긴 아래꼬리 + BUY 방향 컬러 확인 = BUY
- 직전 확정 스윙고점 위로 실제 상향 스윕 후 그 라인 아래에서 마감 + 긴 위꼬리 + SELL 방향 컬러 확인 = SELL
...
- 기준선은 가장 최근 확정 스윙저점이다.
- 스윙저점은 기준 캔들의 저가가 좌우 `SwingDepthBars`개 캔들의 저가보다 낮을 때 확정된다.
- 기본값은 `SwingDepthBars = 3`이다.
- 셋업캔들 자체는 스윙 확정 기준선으로 사용하지 않는다.

### 3.2 SELL 기준 라인

- 기준선은 가장 최근 확정 스윙고점이다.
...
3. 직전 확정 스윙저점 라인이 존재해야 한다.
4. 셋업캔들의 저가는 해당 스윙저점 라인을 `MinFalseBreakoutPoints` 이상 실제로 하향 이탈해야 한다.
5. 이탈 폭 상한 제한은 없다.
6. 셋업캔들의 종가는 해당 스윙저점 라인 위에서 마감해야 한다.
...
1. 차트 주기는 M1이어야 한다.
2. 새 봉이 시작된 시점에 직전 마감봉을 셋업캔들로 평가한다.
3. 직전 확정 스윙고점 라인이 존재해야 한다.
4. 셋업캔들의 고가는 해당 스윙고점 라인을 `MinFalseBreakoutPoints` 이상 실제로 상향 이탈해야 한다.
5. 이탈 폭 상한 제한은 없다.
6. 셋업캔들의 종가는 해당 스윙고점 라인 아래에서 마감해야 한다.
...
| BUY 기준 라인 | 가장 최근 확정 스윙저점 |
| SELL 기준 라인 | 가장 최근 확정 스윙고점 |
