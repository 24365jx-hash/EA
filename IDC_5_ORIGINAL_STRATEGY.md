...
명확한 구조가 진행되다가 직전 스윙저점 또는 직전 스윙고점을 종가로 돌파하지 못하고, 긴 꼬리로 유동성을 확보한 뒤 기준선 안쪽에서 마감할 때 진입한다.

- LH/LL 하락 구조 진행 + 직전 스윙저점 하향 스윕 후 그 라인 위에서 마감 + 긴 아래꼬리 = BUY
- HL/HH 상승 구조 진행 + 직전 스윙고점 상향 스윕 후 그 라인 아래에서 마감 + 긴 위꼬리 = SELL
...
| 직전 저점 돌파 실패 BUY | 셋업캔들 저가가 직전 스윙저점 터치/하향 스윕 후 종가가 저점 위에서 마감 |
| 직전 고점 돌파 실패 SELL | 셋업캔들 고가가 직전 스윙고점 터치/상향 스윕 후 종가가 고점 아래에서 마감 |
