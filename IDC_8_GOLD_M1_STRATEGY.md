...
## 원본 전략 자료 (Official Strategy Document) v3.08
...
| 3 | RSI **52 이상 확인** → **종가 48 미만** (엄격 돌파 또는 구간 이탈) |
| 4 | 종가 9EMA 아래 + 몸통>꼬리 (꼬리가 9EMA 터치해도 인정, RSI 2단계 완료봉 포함 **3봉** 유예) |
| 5 | 사이클당 1회 |
| 6 | 데드크로스 후 SELL만 |
...
| RSI 52/48 2단계 미완료 | **무효** |
| 3봉 유예 초과 진입 | **금지** |
| SL 미설정 | 금지 |

---

*EA 파일: `IDC_8.mq4` v3.08*

### v3.08 변경 (셋업캔들 즉시 진입)

| 항목 | 내용 |
|------|------|
| `g_rsi_seen_above_upper` / `g_rsi_seen_below_lower` | 52/48 엄격 돌파 없이 구간 체류도 1단계 인정 |
| RSI 2단계 완료 | 1단계 확인 후 **종가**가 반대 기준선 밖이면 WAIT_EMA |
| 9EMA 확인 | `low/high` 강제 이탈 제거 — **종가 + 몸통>꼬리**만 검사 |
