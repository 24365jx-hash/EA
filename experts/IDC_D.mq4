...
#property description "GOLD-only M1/M5 EA with 20-pip lock and step trailing."
...
input int    ATRMinPoints             = 35;
input int    ATRMaxPoints             = 450;

input bool   UseDailyProbe            = true;
input int    DailyProbeStartHour      = 15;
input bool   RelaxedTrendProbe        = false;

input bool   PrintDebugLogs           = false;
...
bool CanOpenNewTrade()
{
   ...
   {
      DebugLog("Spread filter blocked entry.");
      return(false);
   }
   ...
   if(MaxDailyLossPercent > 0.0)
   {
      ...
      {
         DebugLog("Daily loss limit blocked entry.");
         return(false);
      }
   }
...
int GetEntrySignal()
{
   ...
   double fastPrev = iMA(TradeSymbol, PERIOD_M1, M1FastEMA, 0, MODE_EMA, PRICE_CLOSE, 2);
   double slowPrev = iMA(TradeSymbol, PERIOD_M1, M1SlowEMA, 0, MODE_EMA, PRICE_CLOSE, 2);
   double rsiNow = iRSI(TradeSymbol, PERIOD_M1, RSIPeriod, PRICE_CLOSE, 1);

   bool trendUp = (m5Fast > m5Slow && m1Close > m5Fast);
   bool trendDown = (m5Fast < m5Slow && m1Close < m5Fast);
   bool probeTrendUp = (trendUp || (RelaxedTrendProbe && m5Fast > m5Slow));
   bool probeTrendDown = (trendDown || (RelaxedTrendProbe && m5Fast < m5Slow));

   bool buyReclaim = (fastNow > slowNow && fastPrev <= slowPrev && rsiNow >= RSIBuyMin);
   bool sellReclaim = (fastNow < slowNow && fastPrev >= slowPrev && rsiNow <= RSISellMax);
   bool buyProbe = false;
   bool sellProbe = false;

   if(UseDailyProbe && CountTodayTrades() == 0 && CurrentSessionHour() >= DailyProbeStartHour)
   {
      buyProbe = (fastNow > slowNow && m1Close > fastNow && rsiNow >= RSIBuyMin);
      sellProbe = (fastNow < slowNow && m1Close < fastNow && rsiNow <= RSISellMax);
   }

   if(trendUp && buyReclaim)
      return(OP_BUY);
   if(trendDown && sellReclaim)
      return(OP_SELL);
   if(probeTrendUp && buyProbe)
      return(OP_BUY);
   if(probeTrendDown && sellProbe)
      return(OP_SELL);
...
void OpenTrade(int orderType)
{
   ...
   if(ticket < 0)
   {
      Print(EA_NAME, ": OrderSend failed. error=", GetLastError());
      ResetLastError();
      return;
   }
